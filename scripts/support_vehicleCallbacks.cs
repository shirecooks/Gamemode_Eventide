package Support_VehicleCallbacks
{
    ...
    //Add small check to disable vehicles from being pushed.
    function Vehicle::onActivate(%vehicle, %activatingObj, %activatingClient, %pos, %vec)
    {
        if(%vehicle.Datablock.cantPush == true)
        {
            return;
        }
        else if(VectorLen(%vehicle.getVelocity()) > 2)
        {
            return;
        }
        ...
        %minigameCanUse = miniGameCanUse(%player, %vehicle);
        if(%minigameCanUse != -1)
        {
            %doFlip = %minigameCanUse;
        }
